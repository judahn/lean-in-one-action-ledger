    ...
    <div>
        ...
        <div className="type-label-sm flex items-center gap-0.5 rounded-full bg-warm-200 p-0.5">
          {(["name", "status"] as Sort[]).map((s) => (
            <button
              ...
              type="button"
              onClick={() => setSort(s)}
              className={`motion-fast h-6 rounded-full px-2.5 ${
                sort === s
                  ? "bg-warm-50 text-warm-900 shadow-card"
                  : "text-warm-500"
              }`}
            >
              By {s}
      ...
      {carried.length > 0 && (
        <Group title="Still open from earlier meetings">
          {carried.map((a) => (
            <Row
              key={a.member.id + a.committed_at.id}
              action={a}
              since={`committed in ${meetingMonth(a.committed_at.held_at)}`}
            />
          ))}
        </Group>
      )}
  ...
  return (
    <li className="flex items-start gap-3 py-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        {action.note && (
          <div className="type-body text-warm-500">{action.note}</div>
        )}
      </div>
