...
export function Sidebar() {
  return (
    <nav className="hidden w-[188px] shrink-0 flex-col gap-6 border-r border-warm-300 pt-6 pr-2 pl-3 lg:flex">
      {SECTIONS.map((section) => (
        <div key={section.label} className="flex flex-col gap-1">
          <div className="type-overline mb-1 px-3 text-warm-500">{section.label}</div>
          {section.items.map(({ label, icon: Icon, active }) => (
            <div
              key={label}
              className={`type-body flex items-center gap-3 rounded-full px-3 py-2 ${
                active ? "bg-warm-300 font-bold" : ""
              }`}
            >
              <Icon className={`size-4 ${active ? "text-burgundy" : ""}`} strokeWidth={2.25} />
              {label}
            </div>
          ))}
        </div>
